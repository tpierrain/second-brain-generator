...
test('extracts the dashed page id from a slugged Notion URL', () => {
  const id = extractPageId('https://www.notion.so/acme/Page-0123abc0b1c24d6e8f0a1b2c3d4e5f60');

  assert.equal(id, '0123abc0-b1c2-4d6e-8f0a-1b2c3d4e5f60');
});

test('strips query parameters', () => {
  const id = extractPageId('https://www.notion.so/Page-0123abc0b1c24d6e8f0a1b2c3d4e5f60?pvs=4');

  assert.equal(id, '0123abc0-b1c2-4d6e-8f0a-1b2c3d4e5f60');
});

test('accepts an already-dashed id and leaves it canonical', () => {
  const id = extractPageId('0123abc0-b1c2-4d6e-8f0a-1b2c3d4e5f60');
...
test('canonicalizes an app.notion.com/p share URL to www.notion.so/<id32>', () => {
  const url = canonicalizeNotionUrl(
    'https://app.notion.com/p/Some-Page-0123abc0b1c24d6e8f0a1b2c3d4e5f60',
  );
...
test('leaves an already-stable www.notion.so URL untouched', () => {
  const url = 'https://www.notion.so/acme/Page-0123abc0b1c24d6e8f0a1b2c3d4e5f60';
...
test('absolutizes a relative Notion page-mention link `/<id32>` to www.notion.so', () => {
  const md = 'Owned by [@Squad A](/0123abc0b1c24d6e8f0a1b2c3d4e5f60) this quarter.\n';

  assert.equal(
    canonicalizeNotionUrlsInMarkdown(md),
    'Owned by [@Squad A](https://www.notion.so/0123abc0b1c24d6e8f0a1b2c3d4e5f60) this quarter.\n',
  );
});
...
test('rewrites broken app.notion.com inline links in the body, preserving stable ones', () => {
  ...
  assert.equal(
    canonicalizeNotionUrlsInMarkdown(md),
    'See [Spec](https://www.notion.so/0123abc0b1c24d6e8f0a1b2c3d4e5f60) ' +
      'and [Hub](https://www.notion.so/Hub-aaaa2ca0b1c24d6e8f0a1b2c3d4e5f60).\n',
  );
});
