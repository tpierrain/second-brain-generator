...
// Aide (n'installe RIEN). Affichée avant les prérequis pour fonctionner même
// sans git/node, et pour qu'un agent puisse inspecter les options sans risque.
function printUsage() {
  console.log(`Usage : node installer.mjs [options]

CRÉE un nouveau dossier cerveau à partir de ce launcher (le launcher reste intact).

Options :
  --name <nom>          Nom du cerveau = nom du dossier créé (défaut : second-brain)
  --dest <dossier>      Dossier parent où créer le cerveau (défaut : ton home → ~/<nom>)
  --owner <nom>         Ton nom (défaut : git config user.name)
  --lang <langue>       Langue par défaut des notes (défaut : français)
  --embedder <choix>    Moteur de recherche : in-process | gemini | ollama
                        (omis → reco adaptative selon la machine)
  --non-interactive     Lance sans poser de questions (REQUIS hors terminal).
                        Alias : --yes, --no-input
  --help, -h            Affiche cette aide et quitte (n'installe rien)

Sécurité : la clé d'API n'est JAMAIS un argument — elle se met dans .env après coup.

Sans terminal interactif ET sans --non-interactive, l'installeur REFUSE de
deviner (pas d'install fantôme avec des valeurs par défaut).`);
}

// --help / -h court-circuite tout, avant même les prérequis : on n'installe rien.
...
// Garde-fou central : on n'installe JAMAIS avec des défauts sans consentement.
// Sans TTY (ex. lancé par un agent) ET sans --non-interactive → REFUS net, avant
// toute création de dossier. Évite l'install fantôme constatée quand l'installeur
// est invoqué sans clavier et sans le flag (ex. pour « inspecter les options »).
...
if (mode === "refuse") {
  err("Refus d'installer avec des valeurs par défaut sans consentement explicite.");
  err("Pas de terminal interactif détecté (stdin non-TTY) et --non-interactive absent.");
  err("→ Lance l'installeur dans un VRAI terminal pour répondre aux questions,");
  err("  ou passe --non-interactive avec au moins --name (voir : node installer.mjs --help).");
  process.exit(1);
}
