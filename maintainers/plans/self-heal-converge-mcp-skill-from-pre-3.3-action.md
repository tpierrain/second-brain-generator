<!-- ════════════════════════════════════════════════════════════════════════ -->
<!-- STATUS: 🟡 IN PROGRESS (2026-06-21) — v3.3.0 SHIP-BLOCKING finding F-B7. Tasks 1+2+3 DONE & green (harness 471/471/0 todo, rag 207/207, local-mirror 87/87, both tsc clean; ADR 0026 amended 055e20f). Commits 02e9a7d→055e20f, NOT pushed. NEXT = Task 4 — re-verify on the real rig (Thomas, Desktop: reset ~/legacy-brain, ONE /update-engine + restart, prove skill+MCP+health-note converge with no 2nd update), then ship gate. -->
<!-- ════════════════════════════════════════════════════════════════════════ -->
...
- [x] **3 — ADR + docs** _(2026-06-21 · 055e20f ADR + QA plan F-B7 section)_
...
- [ ] **5 — F-B7b: the health-check note does NOT converge for a pre-3.3.0 upgrader** _(found 2026-06-21 during Task 4)_
  ...
  - [x] 5g — amend **ADR 0026 in place** (decision B): the note seeds in BOTH modes from a non-sacred staged source — the SAME delivered-files principle as skills/MCP; self-heal CAN seed _(2026-06-21 — §5 rewritten clean (staged `engine-health/`, seed-if-absent both modes, presence-keyed reindex), Consequences "converges at first restart's self-heal", dropped the now-obsolete "seeding in self-heal rejected" alternative, added `engine-health/` to the prior-art delivered-files list; no dated scar per write-adrs-for-a-fresh-reader, the ADR is unshipped)_
