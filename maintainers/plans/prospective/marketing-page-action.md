...
- [ ] **Step 7 — Mainstream board redesign (grand public)** _(started 2026-07-23, branch `docs/marketing-page`)_
  - Why: boards still read like a feature list. Target reader = **non-tech, overloaded** (manager/PM/consultant); register = **calm / relief** (chaos → serenity); rule = **1 idea = 1 board**; kill jargon; demote mechanics to a small **"if you're curious:"** strip. Grammar = **3 tiers** (hero emotion → promise → curious strip). North-star (from 7h): ***Never forget · Never let anyone down · Never drown.***
  ...
  - [x] 7h · **Campaign re-cut with Thomas → the peace-of-mind triptych** _(2026-07-23)_ — the messages, not the mechanics, now lead. Three "never" boards, one ache each, plus a hero lead card carrying all three: **Never forget** (`board-remember` — memory + instant recall, no invention/web) · **Never let anyone down** (`board-commitments` — zero mental load, never late, never drop a ball) · **Never drown** (`board-connect` — plugged into the whole company's shared data, filtered). New north-star: ***Never forget · Never let anyone down · Never drown.*** Boundary agreed: `board-connect` = the *personal* "I'm plugged in, filtered" vecu; `board-mesh` (act 2) stays the *systemic* mesh vision.
    - [x] Wrote the 4 paste-ready prompts (`board-hero` rewritten to carry the 3 pillars + `board-remember` + `board-commitments` + `board-connect`), all in the validated 3-tier grammar + fixed ribbon + anti-leak guard.
    - [x] Reorganized `docs/marketing-image-prompts.md`: campaign on top, **Annex** at the bottom (superseded pilots `stay-on-top`/`clarity`, pre-triptych explainers, under-the-hood tech) so old prompts can't be regenerated by mistake. *(Subsumes the old 7i "propagate grammar" and 7j "move tech to annex".)*
    - [x] **All 4 campaign renders integrated & verified clean** _(2026-07-23 · `04e86dd` hero/remember/commitments, `3491299` connect)_ — no scaffolding leak, ribbons correct, 3-tier grammar respected. `board-connect` regenerated once to fix a dropped letter in its sub-line ("an it filters" → "and it filters"). *(The stale `board-stay-on-top.png` render, 08:15, leaked the pre-fix ribbon — discarded, not fetched.)*
  - [ ] 7i · _(optional)_ lowercase the legacy `SOLID ORANGE pill` phrasing in the annex too (proven safe today, but latent) if legacy boards get regenerated
  - [x] 7j · Wire the campaign boards into `README.md` / `MARKETING.md` _(2026-07-23 · `1ed4de8`)_ — README: refreshed `board-hero` alt to the 3-pillar card + new "What it gives you every day" section with the 3 pillar boards; MARKETING: `board-hero` banner up top + "Three everyday wins" section (never forget · never let anyone down · never drown) with captions.

- [ ] **Step 8 — The technical acts: WHAT-as-software + HOW** _(started 2026-07-23, branch `docs/marketing-page`)_
  - Why: acts 1–2 (WHY + personal WHAT) are done; the technical half needed (a) a **"what is it, as software"** anatomy board Thomas asked for — *"a surcouche over Claude: MCP + scripts + a two-storey constitution — so people see it's software, not just Markdown"* — and (b) the **HOW** wired from the already-prepared material. Verified in code first: the **two-storey constitution IS shipped** — thin sacred `CLAUDE.md` (personalized, `SACRED_FILES`, never clobbered) `@import`s engine-managed `CLAUDE.engine.md` (generic, refreshed by upgrades); propagation to *already-deployed* brains is deferred (locale-aware, per `engine-managed-file-merge-strategy.md`). So the board claims the split honestly, not the propagation.
  - [x] 8a · Confirmed with Thomas the "constitution aux deux étages" = framework (`CLAUDE.engine.md`, upgrade-refreshed) + personal/private (`CLAUDE.md`, sacred); verified it's really built _(2026-07-23)_
  - [x] 8b · New prompt `board-anatomy` ("More than Markdown") — **filed as the WHAT, at software level** (answers *"what IS Kenjaku?"*), opening act 3 and bridging the personal WHAT into the HOW; exploded stack: Claude base slab + Kenjaku software layer (two-storey constitution · local MCP servers · JS/TS scripts · skills · settings.json guardrails · the Markdown vault), each tile annotated with *what it's for*; same house style + anti-leak guard + fixed #E8590C ribbon _(2026-07-23)_
  - [x] 8c · `MARKETING.md` — new section **"What Kenjaku *is*, as software — more than Markdown"** (explicitly the **WHAT** beat, with an arc-marker comment for filing) at the top of act 3 (after the hinge), with the `board-anatomy` slot + the annotated component list _(2026-07-23)_
  - [x] 8d · `MARKETING.md` — surfaced the signature `board-determinism` slot leading "What's in the box" (was referenced nowhere) _(2026-07-23)_
  - [x] 8d-bis · Render iterations on `board-anatomy` _(2026-07-23)_ — added (i) an anti-duplication guard (single MCP / vault / Skills tile; never echo a caption as a floating label — fixed a doubled MCP tile then a doubled `Your vault` + a stray double "constitution" label); (ii) a **semantic colour code** (soft **parme** — a pale tint of the Kenjaku brand parme ~#B884D8 — for the CODE/deterministic tiles: MCP servers · Scripts · settings.json = the Kenjaku software layer; vs plain paper for the Markdown tiles: the two constitution plates · Skills · vault; with a tiny legend), to make "more than Markdown" read at a glance. Chosen parme over blue for brand cohesion + a tighter palette (orange + parme + paper). (iii) Added the missing **RAG engine** piece — a `RAG engine · JS/TS` panel behind `vault-RAG` (on-device EmbeddingGemma · SQLite vector store · incremental index · swappable embedder), joined by a `port → engine` connector so the hexagon story reads (MCP = stable port, engine = swappable adapter); mirrored as a dedicated bullet in `MARKETING.md`. (iv) Hardened the colour code into a **strict per-tile assignment** after the model kept tinting **Skills** parme — Skills is `SKILL.md` (Markdown) so it MUST stay paper; only MCP servers · RAG engine · Scripts · Hooks · settings.json are parme. (v) Pulled **Hooks** out of the Scripts tile into a **separate `Hooks · event-driven` tile** (auto-commit on edit · auto-push on Stop · reconcile at session start) — they're event-driven deterministic automation, not invoked scripts; own bullet in `MARKETING.md`. (vi) SQLite drawn as a database-cylinder icon, single instance. (vii) Anti-leak sweep after "RESTING ON TOP" / "Brain's Rules" layout words got painted + a duplicated "the rules Claude follows" ("rulers" typo): reworded the descriptions and extended the guard. (viii) **Spatial split enforced** — Markdown/paper tiles on the LEFT, code·deterministic/parme tiles on the RIGHT, so the two families read by position as well as colour (CLAUDE slab still spans full width beneath). (ix) When the split flattened the scene into a 2D grid, **locked the ISOMETRIC 3D perspective** (raised tiles with thickness + shadow, exploded-isometric look of the preferred render) and made the left/right bias explicit as "within the same isometric perspective, do NOT switch to a flat grid". (x) After leaks resurfaced in the isometric renders: dropped the inline "SQLite … ONCE only" parenthetical (it got painted) → moved to the meta dedup guard; reworded "At the very BOTTOM" → "At the base" (the word BOTTOM was painted); guard now also forbids a duplicated legend + empty/unlabeled tiles. (xi) **Vault made GOLD** (honey-amber ~#F5D98B) to spotlight the user's data as the treasure — legend becomes 3-item (code · Markdown · your data); vault caption gains `[[wikilinks]]` + an **"Obsidian-ready"** chip (framed as compatibility, not dependency — Obsidian stays an optional viewer). (xii) If space is tight, the **RAG engine / SQLite panel may overhang the board edge** (Thomas's preference) rather than shrinking everything.
  - [x] 8e-anatomy · **`board-anatomy.png` rendered & integrated** _(2026-07-24)_ — validated render (isometric 3D, paper-left / gold-vault / parme-right, 3-item legend, Obsidian-ready + [[wikilinks]], SQLite cylinder, Hooks split out) copied into `docs/img/` and its `<img>` uncommented in `MARKETING.md` (excluded from brain copy via the `board-*` rule). *Note: on re-rolls the long proper noun "EmbeddingGemma" occasionally misspells on the board; the integrated render is clean. A plan-B (drop the board chip to "on-device embeddings (ONNX)", keep the full name only in MARKETING prose) was floated but NOT applied — the chosen render didn't need it, so the prompt still says "EmbeddingGemma".*
  - [x] 8e-determinism · **`board-determinism.png` rendered & integrated** _(2026-07-24)_ — the ⭐ signature board (v2 compo: robot→orange sweep→`vault-search`/`gate` checkpoint on a pipeline + rack, mascot relaxed). Thomas picked this render and hid the duplicated "AI improvising" label himself; copied into `docs/img/` and its `<img>` uncommented in `MARKETING.md`. **Lesson (do NOT repeat): hardening the prompt with inline prose guards ("ONE TIME only", spelling call-outs) BACKFIRED — the model painted the guards onto the board (v3/v4 far worse than the untouched v2). Reverted the prompt to original; stochastic text glitches are fixed by re-rolling, never by adding guard prose.** See memory `nano-banana-verbose-guards-leak`.
  ...
  - [ ] 8e · Render check by Thomas — remaining HOW render `board-hexagon.png` premium PNG (prompt already carries the real MCP tool names, verified against `rag/src/index.ts`), drop in `docs/img/`, verify clean, then uncomment the `<img>` slot. **Keep the prompt as-is (minimal authorized-text list) — re-roll on glitch, don't add prose guards.**
