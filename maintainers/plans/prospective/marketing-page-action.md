...
- [ ] **Step 7 — Mainstream board redesign (grand public)** _(started 2026-07-23, branch `docs/marketing-page`)_
  ...
  - [x] 7h · **Campaign re-cut with Thomas → the peace-of-mind triptych** _(2026-07-23)_ — the messages, not the mechanics, now lead. Three "never" boards, one ache each, plus a hero lead card carrying all three: **Never forget** (`board-remember` — memory + instant recall, no invention/web) · **Never let anyone down** (`board-commitments` — zero mental load, never late, never drop a ball) · **Never drown** (`board-connect` — plugged into the whole company's shared data, filtered). New north-star: ***Never forget · Never let anyone down · Never drown.*** Boundary agreed: `board-connect` = the *personal* "I'm plugged in, filtered" vecu; `board-mesh` (act 2) stays the *systemic* mesh vision.
    ...
    - [x] **3 of 4 campaign renders integrated** _(2026-07-23 · `04e86dd`)_ — `board-hero.png` (rewritten, carries the 3 pillars), `board-remember.png`, `board-commitments.png`: all verified clean (no scaffolding leak, ribbons correct, 3-tier grammar respected). *(The stale `board-stay-on-top.png` render, 08:15, leaked the pre-fix ribbon — discarded, not fetched.)*
    - [ ] **NEXT (Thomas): regenerate `board-connect`** — its render dropped a letter in the sub-line (*"an it filters"* → *"and it filters"*); prompt is already correct, a fresh render fixes it → Claude fetches into `docs/img/`.
  ...
  - [ ] 7j · Wire the finished campaign boards (`hero` + the 3 pillars) into `README.md` / `MARKETING.md` _(deferred until the renders land)_
